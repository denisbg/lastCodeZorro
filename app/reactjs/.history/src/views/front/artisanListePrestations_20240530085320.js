  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (!isPending || refresh) {
      ...
      if (refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        ...
        if (false && filter.category.value) {
        ...
        console.log("CP cpParamsUrlService",cpParamsUrlService);
        console.log("CP paramsUrlService",paramsUrlService);
        console.log("CP condition",condition);
        
        
        const repairmanId = parseInt(slugIdRepairman.split("-").pop());
        let repairman = [];
        let repairmanBenefit = [];
        if (true ||refresh || cpParamsUrlService !== paramsUrlService) {
          ...
          connector({
            ...
            success: (response) => {
              let xServices = [];
              let xPrestations = [];
              console.log("CP RESP'",`${endPoints.ANONYMOUS_SERVICES_CATALOG}?${cpParamsUrlService}`);
console.log("CP RESP'",response.data["hydra:member"]);
