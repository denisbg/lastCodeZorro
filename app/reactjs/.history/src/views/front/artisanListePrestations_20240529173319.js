...
export default function ArtisanListePrestations() {
  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (true || !isPending || refresh) {
      setIsPending(true);
      setFilter(clearErrors(filter));
      setFilterMap(clearErrors(filterMap));

      if (true || refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        let condition = "";
        ...
        //perpage
        const cpParamsUrlService = `page=${parseInt(index) + 1}&itemsPerPage=${perPage}&categories.parent.universe.id=${parent_univ_id}`;
        const repairmanId = parseInt(slugIdRepairman.split("-").pop());
        let repairman = [];
        let repairmanBenefit = [];
        console.log("ALP 969 2");
        if (refresh || cpParamsUrlService !== paramsUrlService) {
          console.log("ALP 969 3");
  ...
  const getUniverse = () => {
    ...
    if (id) {
      connector({
        method: "get",
        url: `${endPoints.ANONYMOUS_UNIVERSES}${id}/categories`,
        success: (response) => {
          const arrayObj = response.data || { categories: [] };
          setUniverse(arrayObj);
console.log("ALP CAT",arrayObj,categories );
          let cpFilter = { ...filter };
