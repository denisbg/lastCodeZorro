...
export default function ArtisanListePrestations() {
  ...
  const openLightbox = (photo, index) => {    setCurrentImage(index);    setStateImage(true);  };
  ...
  const closeLightbox = () => {    setStateImage(false);  };
...
const getUserRepairmanBenefits = (id) => {
 ...
 // affichage de l'entete du reparateur
//
function getBenefitIdFromService(serviceId, repairmanId) {
  ...
  useEffect(() => {
    ...
    */
    ...
    if (typeof serviceId == 'undefined') {
      console.log("ALPa 1.00 le service n'est pas defini (NP)", serviceId, repairmanId);
      setXServiceIsSelected(false);
    }
    else {
        console.log("ALPa 1.01 le service est defini (AP) service et repaimanid ", serviceId, repairmanId);
    if (serviceId && !isAlreadySet) {
        getBenefitIdFromService(serviceId, repairmanId);
        console.log("ALPa 1.02 getBenefitIdFromService", serviceId, repairmanId);
  ...
  useEffect(() => {
    ...
    const universeId = parseInt(slugIdUniverse.split("-").pop());
    if (currentRepairmanId !== repairmanId) {
      // affiche la banner du repairman
      getRepairMan(repairmanId);
      //GetSerRB(repairmanId,universeId); //27/05/2024
    }
    else
  ...
  console.log("ALP STEP x:003 Serviceservice key universe", services);
          let xi = 0;
