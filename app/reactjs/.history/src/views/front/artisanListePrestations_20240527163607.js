...
///api/services/repairman?page=1&itemsPerPage=20
  ...
  const getServiceRepairmanBenefits = (id) => {
    console.log("ALP STEP 0.404 repairmanbenefit loading.. ");
    //url: `${endPoints.ANONYMOUS_USER_REPAIRMAN}/${id}/benefits`,
    connector({
      method: "get",
      url: `api/services/repairman?page=1&itemsPerPage=20`,
      success: (response) => {
  ...
  useEffect(() => {
    ...
    if (currentRepairmanId !== repairmanId) {
      ...
      getServiceRepairmanBenefits(repairmanId); //27/05/2024
    }
    else
