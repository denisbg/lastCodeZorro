      ...
      connector({
        ...
        success: (response) => {
          ...
          if (obj?.deliveryModes && obj.deliveryModes.length) {
            ...
            setState(cpState);
            //console.log("ASLP 1:24 Standard cpState.deliveryModes.options",cpState.deliveryModes);
          }
          if (obj?.user?.id) {
  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (!isPending || refresh) {
      ...
      if (refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        ...
        //console.log("DB1 11:031 parent_univ_id", parent_univ_id);
        ...
        if (refresh || cpParamsUrlService !== paramsUrlService) {
          ...
          connector({
            ...
            success: (response) => {
              ...
              //console.log("DB1 x12:13 DB1 11:06SUCCESS WEB SERVICES DATA  ", response.data["benefits"] );
          ...
          connector({
            ...
            success: (response) => {
              let xServices = [];
              let xPrestations = [];
              //console.log("DB1 12:01 xor DB1 12:08SUCCESS   ", response.data["hydra:member"] );
              //setServices(response.data["hydra:member"] || []);
              setTotalServices(response.data["hydra:totalItems"] || 0);
              //console.log("DB1 12:08SUCCESS extraction de ceux pour lequels une prestation de la artisan existe  ",response.data["hydra:member"]);
              let xi = 0;
              ...
              Object.entries(response.data["hydra:member"]).forEach(entry => {
                ...
                const [key, service] = entry;
                countSrv++;
                console.log("DB1 12:10  parcours de la table service init ",countSrv, service['displayedinfront'] ); 
                // le but est de voir si l'artisan est dans la table benefit(propose ses prestations pour ce service)
                let xorg = 0
                if (service['displayedinfront'] != '1') {
                Object.entries(service).forEach(entry => {
                  const [key, prestations] = entry;
                  xorg++;
                  //console.log("DB1 12:10  parcours de la table service  traitement du service :",xorg,service);


                  if (key === 'benefits') {
                    //console.log("DB1 x12:13   parcours de la table service recherche dans les presta ",prestations);
                    let found = false;
                    Object.entries(prestations).forEach(entry => {
                      const [key, prestation] = entry;
                      //console.log("DB1 12:121aBUGG traitement de la prestation ",prestation);
                      ...
                      Object.entries(repairmanBenefit).forEach(entry => {
                        ...
                        if (value['id'] === prestation['id']) {
                          //console.log("DB1 12:122-----TROUVE-----", xi,"trouvé",value);
                          ...
                          //
                          //value = prestation;
                          console.log('ALP STEP 1.151.8 id repairman is found ***Benefit detail', value);
                          if (value['benefitToDisplay'] !== '0') {
                            found = true;
                            service['benefit_to_display'] = value['id'];
                            //setBenefit(value['id']); COORECTION BUG AFFICHAGE
                            //console.log("DB1 11:06SUCCESS 12:122 PUSH service ",service);
                            //console.log("DB1 11:06SUCCESSPUSH 12:122value ",value);
                            //console.log("DB1 11:06SUCCESS 12:222", "-------");
                      ...
                      if (!found) {
                        //
                        //console.log("DB1 12:122------NON TROUVE  not found", prestation, repairmanBenefit );
                      }
              ...
              //console.log("DB1 x12:13  liste des xPrestations ",xPrestations);
              //console.log("DB1 x12:13  liste des xServices ",xServices);
              //console.log("DB1 x12:13  total xServices ",xi);
          ...
          <ContentPageStyle>
            <Container>
                ...
                <Row>
                  ...
                  <Col>
                    {!xserviceIsSelected && (
                      ...
                      <div style={{ width: '100%', height: '100%', paddingLeft: 20, paddingRight: 20, paddingTop: 10, paddingBottom: 10,
                       background: 'white', boxShadow: '0px 0px 12px rgba(0, 0, 0, 0.10)', borderRadius: 4, flexDirection: 'column', 
                       justifyContent: 'flex-start', alignItems: 'flex-start', gap: 13, display: 'inline-flex' }}>
                        <div style={{ height: 137, flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', gap: 20, display: 'flex' }}>
                          ...
                          <div style={{ justifyContent: 'flex-start', alignItems: 'center', gap: 78, display: 'inline-flex' }}>
                            <div style={{ paddingLeft: 20, paddingRight: 20, paddingTop: 8, paddingBottom: 8, background: '#89B03D', borderRadius: 2, justifyContent: 'center', alignItems: 'center', gap: 10, display: 'flex' }}>
                              <Link onClick={() => document.getElementById('decouvrez')?.scrollIntoView()}>
                                <div style={{ color: 'white', fontSize: 16, fontFamily: 'Poppins', fontWeight: '500', wordWrap: 'break-word' }}>Choisissez une prestations</div>
                ...
                <Row>
                  {xserviceIsSelected && (
                    <Col style={{ overFlow: 'hidden', textOverflow: 'ellipsis' }}  >
                      ...
                      <div style={{
                        width: '100%', height: '100%', padding: 20, background: 'white', boxShadow: '0 4px 51px 0 rgba(182, 172, 251, 0.42)', 
                        borderRadius: 4,
              ...
              <Container className="col-md-8 col-sm-3 col-lg-8" style={{ marginTop: 0 }}>
                <div style={{
                  textAlign: 'center', marginBottom: 25, paddingLeft: 20, color: '#465a61', lineHeight: 2
                }}><div id="decouvrez" style={{ color: '#4E5860', fontSize: 32, fontFamily: 'Poppins', fontWeight: '700', wordWrap: 'break-word' }}>Découvrez mes prestations </div></div>
