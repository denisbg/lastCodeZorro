...
export default function ArtisanListePrestations() {
  ...
  const getUserRepairmanBenefits = (id) => {
    ...
    connector({
      ...
      success: (response) => {
        ...
        console.log("ALP STEP 0.599 servuce repairmanbenefit is loaded.. ", arrayObj);
  ...
  const getRepairMan = (repairmanId) => {
    ...
    connector({
      ...
      success: (response) => {
       ...
       //2024 getUserRepairmanBenefits(repairmanId);
       //2024 getUserRepairmanBenefitsUniverses(repairmanId);
  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (!isPending || refresh) {
     ...
     //2024 if (refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        let condition = "";
        ...
        if (refresh || cpParamsUrlService !== paramsUrlService) {
          ...
          connector({
            ...
            success: (response) => {
              ...
              setXServices(services);
              
              //2024 setXPrestations(xPrestations);
              setCompletDataClicked(xPrestations);

             //2024 setTotalServices(xi);
...
console.log("ALP ARRAYOBJ:01",arrayObj)
          let cpFilter = { ...filter };
  ...
  const handleParentEvent = (data, completeData) => {
    ...
    if (currentBenefit === data['benefit_to_display']) {
      ...
      //2024getServices(true, 0);
      //2024getBenefit(data['benefit_to_display']);
