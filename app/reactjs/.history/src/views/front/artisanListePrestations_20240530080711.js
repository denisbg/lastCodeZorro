  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (!isPending || refresh) {
      ...
      if (refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        ...
        //perpage
        const cpParamsUrlService = `page=${parseInt(index) + 1}&itemsPerPage=114&categories.parent.universe.id=${parent_univ_id}${condition}`;
        console.log("cpParamsUrlService",cpParamsUrlService);
        const repairmanId = parseInt(slugIdRepairman.split("-").pop());
        let repairman = [];
