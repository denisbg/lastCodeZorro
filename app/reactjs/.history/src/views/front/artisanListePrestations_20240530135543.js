  ...
  const getServices = (refresh = true, index = null) => {
    ...
    if (!isPending || refresh) {
      ...
      if (refresh || (filterMap.codePostal.latitude && filterMap.codePostal.value)) {
        ...
        //perPage=100000;
        const cpParamsUrlService = `itemsPerPage=5000&categories.parent.universe.id=${parent_univ_id}${condition}`;
        ...
        if (refresh || cpParamsUrlService !== paramsUrlService) {
          ...
          connector({
            method: "get",
            url: `${endPoints.ANONYMOUS_USER_REPAIRMAN}/${repairmanId}/page=1&itemsPerPage=5000`,
            success: (response) => {
          ...
          connector({
            method: "get",
            url: `${endPoints.ANONYMOUS_SERVICES_CATALOG}?${cpParamsUrlService}&PerPage=5000`,
            success: (response) => {
              let xServices = [];
              let xPrestations = [];
